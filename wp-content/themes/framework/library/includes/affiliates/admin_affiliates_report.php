...
}else
{
	?>
    <br />
	<h2><?php _e(AFF_MEMBERS_PAGE_TITLE); ?></h2>
	<?php
	global $wpdb;
	...
	if($userinfo)
	{
		...
		<table width="100%"  class="widefat post fixed" >
		  <thead>
			 <tr>
				<th class="title"><?php _e(AFF_USER_NAME_TEXT); ?> </th>
				<th class="title"><?php _e(AFF_TOTAL_SALES_AMOUNT_TEXT); ?> </th>
				<th class="title"><?php _e(AFF_TOTAL_SHARE_AMT_TEXT); ?> </th>
			</tr>
			<?php
			foreach($userinfo as $userinfoObj)
			{
				...
				if($user_role['affiliate'])
				{
					...
					?>
					<tr>
						<td class="row1" ><a href="<?php echo site_url();?>/wp-admin/admin.php?page=affiliates_settings&user_id=<?php echo $userinfoObj->ID;?>"><?php echo $userinfoObj->user_login;?></a>
