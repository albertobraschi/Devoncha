...
<?php get_header(); ?>
 ...
 <div id="wrapper" class="container_16 clearfix">
    <div id="content" class="grid_11 fl content_left">
      	
        <h1 class="head">
      <?php _e(MY_SALES_REPORT_TEXT); ?>
    </h1>
    ...
    <form action="" method="post" name="frm_srch" id="frm_srch">
     <?php _e(SEARCH_BY_TEXT);?> : 
     <?php _e(DATE_FROM_TEXT);?>  <input type="text" name="srch_st_date"  id="srch_st_date"  value="<?php echo $_REQUEST['srch_st_date']; ?>" size="10"/>
    ...
    &nbsp;<img src="<?php echo bloginfo('template_directory');?>/images/cal.gif" alt="Calendar" onclick="displayCalendar(document.frm_srch.srch_end_date,'yyyy-mm-dd',this)" style="cursor: pointer;" align="absmiddle" border="0">
     ...
     <tr>
	    <td class="title"><?php _e(DATE_TEXT); ?> </td>
        <td class="title"><?php _e(TRANSACTION_ID_TEXT); ?> </td>
        <td class="title"><?php _e(PAYMENT_STATUS_TEXT); ?> </td>
        <td class="title"><?php _e(ITEM_NAME_TEXT); ?> </td>
        <td align="center" class="title"><?php _e(AFF_QTY_TEXT); ?> </td>
        <td align="right" class="title"><?php _e(AFF_AMOUNT_TEXT); ?> </td>
        <td align="center" class="title"><?php _e(CURRENCY_TEXT); ?> </td>
        <td align="right" class="title"><?php _e(AFFILIATE_SHARE_TEXT); ?> </td>
    </tr>
    <?php
	$record_count = 0;
	...
	{
	?>
    <tr><td colspan="8"> <h4> <?php _e(NO_RECORDS_MSG);?> </h4> </td></tr>
    <?php
	}else
	{
	?>
    <tr><td  colspan="3" class="row1">&nbsp;</td>
    <td align="right" class="row1"><b><?php _e(AFF_TOTAL_TEXT);?></b></td><td align="center" class="row1" ><b><?php echo $product_qty;?></b></td><td align="right" class="row1" ><b><?php echo $General->get_amount_format($total_order_amt,0);?></b></td><td align="right" class="row1" >&nbsp;</td><td align="right" class="row1" ><b><?php echo $General->get_amount_format($share_total,0);?></b></td></tr>
    <tr>
      <td  colspan="8" align="right"  >    <a href="<?php echo site_url();?>/?ptype=account&report_export=1<?php echo $params?>" target="_blank" class="i_excel" ><?php _e(EXPORT_TO_EXCEL_TEXT);?></a>  </td>
      </tr>
    <?php
	}
	?>
