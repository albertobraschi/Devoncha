...
function addtocart (e) {
	...
	if (options && options_default) {
		...
		if (!selections) {
			if (!options_required) options_required = "You must select the options for this item before you can add it to your ptstoreing cart.";
	...
	if ($(form).find('input.addtocart').hasClass('ajax')) {
		ptstoreCartAjaxRequest(form.action,$(form).serialize());
	} else {
		form.submit();
	}
 ...
 **/
var ptstoreCartAjaxRequest = function (url,data,response) {
	cartajax(url,data,response);
}

/**
 * ptstoreCartAjaxHandler ()
 * Overridable wrapper function to handle cartajax responses.
 * Developers can recreate this function in their own
 * custom JS libraries to change the way the cart response
 * is processed and displayed to the ptstoreer.
 **/
var ptstoreCartAjaxHandler = function (cart) {
	(function($) {
		var display = $('#ptstore-cart-ajax');
		...
		if ($('#ptstore-sidecart-items').length > 0) {
			$('#ptstore-sidecart-items').html(cart.Totals.quantity);
			$('#ptstore-sidecart-total').html(asMoney(cart.Totals.total));			
		} else {
			$('.widget_ptstorecartwidget p.status').html('<a href="'+cart.url+'"><span id="ptstore-sidecart-items">'+cart.Totals.quantity+'</span> <strong>Items</strong> &mdash; <strong>Total</strong> <span id="ptstore-sidecart-total">'+asMoney(cart.Totals.total)+'</span></a>');
		}
		display.slideDown();
	})(jQuery)	
}
...
function catalogViewHandler () {
	(function($) {
		...
		$(display).find('ul.views li button.list').click(function () {
			...
			document.cookie = 'ptstore_catalog_view=list; expires='+expires+'; path=/';
		});
		$(display).find('ul.views li button.grid').click(function () {
			$(display).removeClass('list').addClass('grid');
			document.cookie = 'ptstore_catalog_view=grid; expires='+expires+'; path=/';
		});
...
/**
 * cartHandlers ()
 * Adds behaviors to ptstoreing cart controls
 **/
	...
	(function($) {
		if (helpurl) {			
			...
			links.each(function (index,link) {
				...
				if (href && href.match(new RegExp(/(.*?)=ptstore\/help$/))) {
					href = href.replace(new RegExp(/(.*?)=ptstore\/help$/),helpurl);
					$(link).attr('href',href);
...
function ptstore_debug () {
	(function($) {
		var overlay = $('<div id="debug" class="ptstore overlay"></div>').appendTo(document.body);
		var debug = $('<div id="debug" class="ptstore"></div>').appendTo(document.body);
		$('<h3>ptstore Debug Console</h3>').appendTo(debug);
		if (memory_profile) {
			$('<h4></h4>').html('Memory:').appendTo(debug);
			$('<p></p>').html(memory_profile).appendTo(debug);
		}
		if (wpquerytotal || ptstorequerytotal) {
			$('<h4></h4>').html('Queries:').appendTo(debug);
			$('<p></p>').html('WP Total: '+wpquerytotal+'<br />ptstore Total: '+ptstorequerytotal).appendTo(debug);
		}
...
function ptstore_gallery (id,evt) {
	(function($) {
		if (!evt) evt = 'click';
		var gallery = $(id);
