...
if (!class_exists('themeui')) {
	class themeui {
		...
		{?>
		<a href="<?php echo site_url('/?ptype=account'); ?>"><?php _e(MY_ACCOUNT_TEXT);?></a>
        <?php
		}
		
		function get_logout_link($css='')
		{?>
		<a href="<?php global $General; echo $General->get_url_login(site_url('/?ptype=login&amp;action=logout')); ?>" class="<?php echo $css;?>"><?php _e(LOGOUT_TEXT);?></a>
        <?php
		}
		...
		{?>
		<a href="<?php global $General; echo $General->get_url_login(site_url('/?ptype=login')); ?>" class="<?php echo $css;?>"><?php _e(REGISTER_TEXT);?></a>
        <?php
		}
		
		function get_checkout_link($css='')
		{?>
		<a  href="<?php echo site_url('/?ptype=cart'); ?>" class="<?php echo $css;?>"><?php _e(CHECKOUT_TEXT);?> &raquo;</a>
        <?php
		}
		function get_affiliate_link($css='')
		{?>
		<a href="<?php echo site_url('/?ptype=setasaff');?>" class="<?php echo $css;?>"><?php _e('Click here');?> &raquo;</a>
        <?php
		}
		...
		function get_shoppingcart_info_header()
		{
            ...
            <div class="header_cart">
                 <?php _e('Você tem');?> <a href="<?php echo site_url('/?ptype=cart'); ?>"><strong> <span id="cart_information_span"><?php echo $itemsInCartCount . "(".$General->get_currency_symbol()."$cartAmount)";?></span></strong></a> <?php _e(SHOPPING_CART_CONTENT_TEXT);?>   | <span class="checkout"> <?php $themeUI->get_checkout_link();?> </span>                 
            </div> <!-- cart #end -->
			<?php
            }
		}
